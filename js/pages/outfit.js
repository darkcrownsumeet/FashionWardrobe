...
const OutfitPage = (() => {
    ...
    function _renderCategory(title, category, items, savedIds) {
        ...
        for (let i = 0; i < items.length; i += 2) {
            ...
            const cardsInPage = pair.map((item, offsetIdx) => {
                ...
                return `
                <div tabindex="0" role="button" class="selection-card group relative flex flex-col cursor-pointer transition-all duration-700 ease-[var(--ease-out-expo)] border-[4px] h-full w-full overflow-hidden ${classState}" data-id="${item.id}" data-category="${category}">
                    <!-- Background Image -->
                    <img loading="lazy" alt="${escapeHtml(item.name)}" src="${item.image}" class="absolute inset-0 w-full h-full object-cover transition-all duration-700 ease-[var(--ease-out-expo)] group-hover:grayscale-0 group-[.active]:grayscale-0 ${imgState}" ${item.objectPosition ? `style="object-position: ${item.objectPosition};"` : ''} onerror="this.onerror=null; this.src='https://image.pollinations.ai/prompt/${encodeURIComponent(item.name + ' fashion isolated')}?width=300&height=400&nologo=true';"/>
                    ...
                    <div class="absolute bottom-0 left-0 w-full p-4 sm:p-6 lg:p-8 flex flex-col gap-1 z-10 transform translate-y-4 transition-transform duration-500 ease-[var(--ease-out-expo)] group-hover:translate-y-0 group-[.active]:translate-y-0">
                        <span class="text-xl lg:text-3xl font-extrabold uppercase tracking-tighter text-white">${escapeHtml(item.name)}</span>
                    </div>
                </div>`;
            }).join('');
    ...
    function render() {
        ...
        const buildLink = (cat, title) => `
            <button class="whitespace-nowrap px-6 py-4 font-mono text-xs uppercase tracking-widest border-b-[2px] transition-all duration-300 cat-link ${cat === 'topwear' ? 'border-brand text-brand' : 'border-transparent text-foreground/50 dark:text-background/50 hover:text-foreground dark:hover:text-background'}" data-target="cat-${cat}">
                ${title}
            </button>
        `;
...
<div class="h-screen w-screen bg-background dark:bg-foreground text-foreground dark:text-background flex flex-col selection:bg-brand selection:text-brand-foreground overflow-hidden">
    ...
    <nav class="sticky top-0 z-50 flex items-baseline justify-between border-b border-foreground dark:border-background bg-background dark:bg-foreground px-6 py-4">
        ...
        <div class="flex items-center gap-8 font-mono text-[10px] uppercase tracking-widest">
            <span class="hidden sm:inline-block border border-foreground dark:border-background px-3 py-1">INDEX 04 / 06</span>
            <button class="transition-colors hover:text-brand flex items-center gap-2" aria-label="Replay Walkthrough" onclick="if(window.Walkthrough) window.Walkthrough.startOutfitTour()" title="Replay Walkthrough">
    ...
    <main class="flex-grow flex flex-col lg:flex-row border-b border-foreground dark:border-background h-full overflow-hidden">
        ...
        <div class="w-full lg:w-[35%] h-[30%] lg:h-full flex flex-col justify-center border-b lg:border-b-0 lg:border-r border-foreground dark:border-background p-6 lg:p-12 relative overflow-hidden">
            <div class="mt-2 lg:mt-8 z-10">
                <span class="font-mono text-[10px] uppercase tracking-[0.3em] text-brand mb-2 lg:mb-6 block">Step 04 / 05</span>
                ...
                <div class="flex flex-col border-[2px] border-foreground dark:border-background bg-background dark:bg-foreground shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,1)] w-full max-w-[280px] transition-all duration-300" id="category-counters">
                    <!-- Header -->
                    <div class="px-5 py-3 border-b-[2px] border-foreground dark:border-background bg-foreground/5 dark:bg-background/5 flex justify-between items-center">
                        <span class="font-mono text-[10px] uppercase tracking-[0.2em] font-bold text-muted-foreground">OUTFIT SUMMARY</span>
                        <span class="material-symbols-outlined text-[14px] text-foreground/50 dark:text-background/50">receipt_long</span>
                    </div>
                    <!-- Rows -->
                    <div class="flex flex-col">
                        ${['Topwear', 'Outerwear', 'Bottomwear', 'Footwear', 'Accessories'].map((cat, index) => {
                            ...
                            return `
                            <div class="flex items-center justify-between gap-8 px-5 py-3 ${!isLast ? 'border-b border-foreground/20 dark:border-background/20' : ''} transition-colors hover:bg-foreground/5 dark:hover:bg-background/5">
                                <span class="font-mono text-[10px] uppercase tracking-[0.2em] font-bold text-foreground/70 dark:text-background/70">${cat}</span>
                                <span class="font-mono text-lg font-black ${count > 0 ? 'text-brand' : 'text-foreground/30 dark:text-background/30'}" id="counter-${key}">${count}</span>
                            </div>
                            `;
                        }).join('')}
                    </div>
        ...
        <div class="w-full lg:w-[65%] h-[70%] lg:h-full bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAiIGhlaWdodD0iMjAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGNpcmNsZSBjeD0iMSIgY3k9IjEiIHI9IjEiIGZpbGw9InJnYmEoMCwgMCwgMCwgMC4wNSkiLz48L3N2Zz4=')] flex flex-col relative" id="lookbook-container">
            ...
            <div class="z-40 bg-background/90 dark:bg-foreground/90 backdrop-blur-md border-b border-foreground/20 dark:border-background/20 flex overflow-x-auto no-scrollbar shrink-0">
            ...
            <div class="flex-grow relative overflow-hidden">
                <!-- Carousel Arrows -->
                <button id="prev-btn-carousel" aria-label="Previous Page" class="absolute left-4 lg:left-8 top-1/2 -translate-y-1/2 z-50 w-12 h-12 bg-background/80 dark:bg-foreground/80 backdrop-blur border border-foreground dark:border-background flex items-center justify-center rounded-full hover:bg-foreground dark:hover:bg-background hover:text-background dark:hover:text-foreground transition-colors disabled:opacity-0 disabled:cursor-not-allowed hidden">
                    <span class="material-symbols-outlined text-2xl">chevron_left</span>
                </button>
                <button id="next-btn-carousel" aria-label="Next Page" class="absolute right-4 lg:right-8 top-1/2 -translate-y-1/2 z-50 w-12 h-12 bg-background/80 dark:bg-foreground/80 backdrop-blur border border-foreground dark:border-background flex items-center justify-center rounded-full hover:bg-foreground dark:hover:bg-background hover:text-background dark:hover:text-foreground transition-colors disabled:opacity-0 disabled:cursor-not-allowed hidden">
    ...
    <footer class="bg-background dark:bg-foreground flex flex-col md:flex-row items-stretch border-t border-foreground dark:border-background shrink-0 z-50 relative">
        <div class="flex-1 p-6 flex items-center justify-between md:justify-start gap-12 border-b md:border-b-0 md:border-r border-foreground dark:border-background">
            <button class="font-mono text-[10px] uppercase tracking-[0.3em] text-foreground/50 dark:text-background/50 hover:text-foreground dark:hover:text-background transition-colors flex items-center group" onclick="window.Router.navigate('/style')">
        ...
        items.forEach(item => {
            const toggleSelection = () => {
                ...
                if (catCounterEl) {
                    ...
                    if (catCount > 0) {
                        catCounterEl.classList.remove('text-foreground/30', 'dark:text-background/30');
                        catCounterEl.classList.add('text-brand');
                    } else {
                        catCounterEl.classList.add('text-foreground/30', 'dark:text-background/30');
