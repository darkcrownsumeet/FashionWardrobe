...
const ArchivePage = (() => {
    ...
    function render() {
        return `
<div class="min-h-screen bg-background text-foreground selection:bg-brand selection:text-brand-foreground" id="archive-wrapper">
    <!-- Top Nav -->
    <nav class="sticky top-0 z-50 flex items-baseline justify-between border-b border-foreground bg-background px-6 py-4">
        ...
        <div class="flex items-center gap-8 font-mono text-[10px] uppercase tracking-widest">
            <span class="hidden sm:inline-block border border-foreground px-3 py-1">ARCHIVE</span>
    ...
    <main class="px-6 py-12 lg:py-20 max-w-[1440px] mx-auto flex-grow w-full flex flex-col">
        <div class="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-foreground/10 pb-6 animate-[reveal_0.8s_var(--ease-out-expo)_both]">
            <div>
            ...
            <div class="flex flex-col items-end gap-4" id="archive-sort-container">
                ...
                <div class="flex border-[2px] border-foreground">
                    <button id="sort-newest" class="px-4 py-2 font-mono text-[10px] font-bold tracking-widest uppercase transition-colors bg-foreground text-background" onclick="ArchivePage.setSort('newest')">NEWEST</button>
                    <button id="sort-score" class="px-4 py-2 font-mono text-[10px] font-bold tracking-widest uppercase transition-colors bg-background text-foreground hover:bg-foreground/10" onclick="ArchivePage.setSort('score')">SCORE</button>
                </div>
            </div>
        </div>

        <div id="archive-grid" class="flex-grow grid grid-cols-1 gap-0 border-t border-l border-foreground bg-background md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 content-start animate-[reveal_0.8s_var(--ease-out-expo)_both] [animation-delay:150ms]">
            <!-- Rendered via JS -->
        </div>
    </main>

    <!-- Footer matching landing -->
    <footer class="px-6 py-12 border-t border-foreground">
    ...
    function init() {
        ...
        if (savedOutfits.length === 0) {
            grid.innerHTML = `
                <div class="col-span-full bg-background py-16 flex flex-col items-center justify-center text-center animate-[reveal_0.8s_var(--ease-out-expo)_both]">
            ...
            `;
            grid.className = 'w-full animate-[reveal_0.8s_var(--ease-out-expo)_both] [animation-delay:150ms] border-t border-foreground/10 mt-8';
            return;
        }

        grid.className = 'flex-grow grid grid-cols-1 gap-0 border-t border-l border-foreground bg-background md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 content-start animate-[reveal_0.8s_var(--ease-out-expo)_both] [animation-delay:150ms]';
        ...
        grid.innerHTML = savedOutfits.map((outfit, index) => {
            ...
            return `
            <div class="archive-card group flex flex-col bg-background p-6 transition-colors hover:bg-secondary relative border-r border-b border-foreground" id="archive-card-${outfit.id}">
                ...
                <div class="mb-6 aspect-[3/4] w-full overflow-hidden bg-[var(--neutral-100)] relative border-[2px] border-transparent group-hover:border-brand transition-colors duration-500 cursor-pointer" onclick="ArchivePage.viewOutfit('${outfit.id}')" title="View Details">
                    ...
                    <div class="absolute bottom-4 right-4 bg-background/90 backdrop-blur-sm border border-foreground px-3 py-2 font-mono text-[10px] font-bold">
                    ...
                    <div class="absolute inset-0 bg-brand/0 group-hover:bg-brand/20 transition-colors flex items-center justify-center">
                        <span class="bg-background text-foreground font-mono text-[10px] uppercase tracking-widest px-4 py-2 border border-foreground opacity-0 group-hover:opacity-100 transition-all translate-y-4 group-hover:translate-y-0 shadow-lg">View Look</span>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap gap-2 mt-auto">
                    ${(outfit.items || []).slice(0, 3).map(i => `<span class="px-2 py-1 bg-foreground/5 text-foreground border border-foreground/10 text-[8px] font-mono uppercase truncate max-w-[120px]" title="${i.name.replace(/'/g, "&#39;")}">${i.name}</span>`).join('')}
                    ${(outfit.items && outfit.items.length > 3) ? `<span class="px-2 py-1 bg-foreground/5 text-foreground border border-foreground/10 text-[8px] font-mono uppercase">+${outfit.items.length - 3}</span>` : ''}
                </div>
            </div>
            `;
        }).join('');
    ...
    function setSort(mode) {
        currentSort = mode;
        document.getElementById('sort-newest').className = mode === 'newest' ? 'px-4 py-2 font-mono text-[10px] font-bold tracking-widest uppercase transition-colors bg-foreground text-background' : 'px-4 py-2 font-mono text-[10px] font-bold tracking-widest uppercase transition-colors bg-background text-foreground hover:bg-foreground/10';
        document.getElementById('sort-score').className = mode === 'score' ? 'px-4 py-2 font-mono text-[10px] font-bold tracking-widest uppercase transition-colors bg-foreground text-background' : 'px-4 py-2 font-mono text-[10px] font-bold tracking-widest uppercase transition-colors bg-background text-foreground hover:bg-foreground/10';
        init();
    }
