...
const WishlistPage = (() => {
    function render() {
        ...
        const cardsHtml = wishlist.length === 0
            ? `<div class="col-span-full bg-background py-16 flex flex-col items-center justify-center text-center animate-[reveal_0.8s_var(--ease-out-expo)_both] border-t border-foreground/10 mt-8 w-full">
            ...
            <div class="archive-card group flex flex-col bg-background p-6 transition-colors hover:bg-secondary relative border-r border-b border-foreground" id="wishlist-card-${item.id}">
                ...
                <div class="mb-6 aspect-[3/4] w-full overflow-hidden bg-[var(--neutral-100)] relative border-[2px] border-transparent group-hover:border-brand transition-colors duration-500 cursor-pointer" onclick="window.open('${item.affiliateUrl || '#'}', '_blank')" title="Shop Now">
                    ...
                    <div class="absolute inset-0 bg-brand/0 group-hover:bg-brand/20 transition-colors flex items-center justify-center">
                        <span class="bg-background text-foreground font-mono text-[10px] uppercase tracking-widest px-4 py-2 border border-foreground opacity-0 group-hover:opacity-100 transition-all translate-y-4 group-hover:translate-y-0 shadow-lg">Shop Now</span>
                    </div>
                </div>
                <h3 class="text-xl font-extrabold uppercase tracking-tighter transition-colors group-hover:text-brand cursor-pointer" onclick="window.open('${item.affiliateUrl || '#'}', '_blank')">${item.name}</h3>
                <p class="mt-2 font-mono text-[10px] uppercase text-brand mb-4 font-bold">${item.category || 'FASHION'}</p>
                <div class="flex flex-wrap gap-2 mt-auto border-t border-foreground/10 pt-4 items-center justify-between">
                    <span class="font-mono text-[10px] uppercase tracking-widest text-muted font-bold">EST. PRICE</span>
                    <span class="font-mono text-sm font-bold text-foreground">$${item.price || 0}</span>
                </div>
            </div>
            `).join('');

        return `
<div class="min-h-screen bg-background text-foreground selection:bg-brand selection:text-brand-foreground">
    <!-- Top Nav -->
    <nav class="sticky top-0 z-50 flex items-baseline justify-between border-b border-foreground bg-background px-6 py-4">
        ...
        <div class="flex items-center gap-8 font-mono text-[10px] uppercase tracking-widest">
            <span class="hidden sm:inline-block border border-foreground px-3 py-1">WISHLIST</span>
    ...
    <main class="px-6 py-12 lg:py-20 max-w-[1440px] mx-auto flex-grow w-full flex flex-col">
        ...
        <div class="flex-grow ${wishlist.length > 0 ? 'grid grid-cols-1 gap-0 border-t border-l border-foreground bg-background md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 content-start' : 'w-full'} animate-[reveal_0.8s_var(--ease-out-expo)_both] [animation-delay:150ms]">
            ${cardsHtml}
        </div>
    </main>

    <!-- Footer matching landing -->
    <footer class="px-6 py-12 border-t border-foreground mt-auto">
